...
| `PATCH /api/runs/:id/status` | 200 with the updated run; allowed: `planned->running`, `running->done` | 404 `{ error: "run not found", id }`; 409 `{ error: "invalid transition", from, to }` |
